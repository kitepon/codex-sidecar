...
- 同一エラーの実発生時に`product_version`を更新し、初回版が残り続ける不具合を修正する。
- runtime storeとsnapshotをschema 3へ移行する。旧schema 1・2の単発記録は版を保持し、複数回の旧集約は最終発生版を復元できないため`unknown`にする。schema 1の解消取消しとsequence移行は維持する。
- consumerを先に更新し、旧writerを終了してから新writerへ切り替える。snapshotは状態を書き換えず、次のロック内更新でschema 3を保存する。保存後に旧版へ戻す場合は製品バックアップを復元し、schemaを手作業で書き換えない。
...
- releaseの同梱検査をnpm 12のJSON出力形式とWindowsのnpm shim起動に対応させ、従来の配列形式も維持する。
